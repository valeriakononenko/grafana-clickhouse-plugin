...
do
  echo "$N"
	INPUT_DATA=$(echo "SELECT (toDateTime(subtractMinutes(now(), $N)), 'user1', rand()), (toDateTime(subtractMinutes(now(), $N)), 'user2', rand() * 1.5), (toDateTime(subtractMinutes(now(), $N)), 'user3', rand() * 2)" | curl -s "http://localhost:8123" -H "X-ClickHouse-User: $USER" -H "X-ClickHouse-Key: $PASSWORD" --data-binary @-)
